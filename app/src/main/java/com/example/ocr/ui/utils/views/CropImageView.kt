...
import com.example.ocr.ui.utils.CropUtils.getPointsDistance
import com.example.ocr.ui.utils.PerspectiveTransformation
import com.example.ocr.ui.utils.extensions.toBitmap
import com.example.ocr.ui.utils.extensions.toMat
...
//            .scale(drawableWidth.toInt() * MAG_FACTOR, drawableHeight.toInt() * MAG_FACTOR)
    ...
    private fun drawMagnifier(canvas: Canvas) {
        if (draggingPoint != null) {
            ...
            val dragX = getViewPointX(draggingPoint!!)
            val dragY = getViewPointY(draggingPoint!!)
            ...
            val pointsDistance: Double = getPointsDistance(dragX, dragY, 0f, 0f)
...
//                dragY += drawableTop / MAG_FACTOR
    ...
    fun getCroppedImage(): Bitmap? {
        if (isValidCrop()) {
            ...
            //Crop points must be in correct order tl, tr, br, bl
            rectangle.fromArray(
                Point((cropPoints[0].x).toDouble(), (cropPoints[0].y).toDouble()),
                Point((cropPoints[1].x).toDouble(), (cropPoints[1].y).toDouble()),
                Point((cropPoints[2].x).toDouble(), (cropPoints[2].y).toDouble()),
                Point((cropPoints[3].x).toDouble(), (cropPoints[3].y).toDouble())
            )
            val dstMat = PerspectiveTransformation.transform(drawable.toBitmap().toMat(), rectangle)
            return dstMat.toBitmap()
        }
        return null
    }

    //The crop is valid if and only if the line color is teal_700
    private fun isValidCrop(): Boolean =
        linePaint.color == ContextCompat.getColor(context, R.color.teal_700)
